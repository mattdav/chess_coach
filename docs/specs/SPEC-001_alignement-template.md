...
- [ ] `.cruft.json` présent, `cruft check` s'exécute sans erreur bloquante
- [ ] `uv run pre-commit run --all-files` passe et est idempotent
- [ ] `uv run inv lint` passe à zéro
- [ ] `uv run inv test` passe
- [ ] `uv run inv docs` build Sphinx sans erreur depuis `docs/code/`
- [ ] `CLAUDE.md` ne contient plus aucune référence à Lichess collector,
...
- [ ] Doublon `src/chess_coach/bin/py.typed` supprimé
- [ ] `_get_package_dir` gère `StopIteration`
