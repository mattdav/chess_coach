...
# Guide de contribution — chess_coach
...
Ce projet charge sa configuration via [python-dotenv](https://github.com/theskumar/python-dotenv)
(`.env`) et un fichier YAML dédié au profil joueur (`src/chess_coach/config/player_config.yaml`,
lu par `chess_coach.bin.player_config`).

### Règles

- Les secrets et valeurs locales vont dans `.env` (jamais commité, dans `.gitignore`)
- Le profil joueur (nom, elo, filtres) va dans `src/chess_coach/config/player_config.yaml`
...
├── config/     ← player_config.yaml (profil joueur) et son chargement
